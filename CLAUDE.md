...
1. Reads `GITHUB_TOKEN`, `GITHUB_USERNAME`, and `PLAN_LIMIT` from the script, then falls back to `COPILOT_TOKEN`, `COPILOT_USERNAME`, and `COPILOT_PLAN` from the environment or shell rc files
...
Users can either edit these variables directly in the script or rely on the `COPILOT_*` environment variables:

```bash
GITHUB_TOKEN=""
GITHUB_USERNAME=""
PLAN_LIMIT=""  # 50 (Free), 300 (Pro), 1500 (Pro+)
```
